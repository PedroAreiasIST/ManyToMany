...
// ============================================================================
// CONSOLIDATED SPARSE MATRIX LIBRARY
// All CSR-related functionality in a single file:
//   - CSR class (core sparse matrix)
//   - PardisoSolver (direct solver via Intel MKL)
//   - CSRIterativeSolvers (BiCGSTAB, eigenvalues)
//   - CuSparseBackend (GPU acceleration via NVIDIA cuSPARSE)
//   - SparseBackendFactory (GPU decision logic)
//   - HybridScheduler (CPU/GPU dispatch)
//   - OptimizedSpMV / OptimizedSpMM (SIMD sparse kernels)
//   - Supporting types (SolverResult, SolverException, MatrixStatistics, etc.)
// ============================================================================
...
// Shared TryGetExport used by both PardisoSolver and CuSparseBackend
// ============================================================================

file static class NativeExportHelper
{
    [MethodImpl(AggressiveInlining)]
    public static bool TryGetExport<T>(IntPtr handle, string name, out T? del) where T : Delegate
    {
...
// ============================================================================
// DIAGONAL CACHE (file-local helper for iterative solvers)
// ============================================================================
...
// ============================================================================
// ITERATIVE SOLVERS (extension methods on CSR)
// ============================================================================
...
internal class PardisoSolver : IDisposable
{
    ...
    private static void LoadPardisoFunctions()
    {
        try
        {
            ...
            {
                if (NativeExportHelper.TryGetExport(mklHandle, "pardisoinit", out _pardisoinit) &&
                    NativeExportHelper.TryGetExport(mklHandle, "pardiso", out _pardiso))
                {
                    _isAvailable = true;
...
internal sealed class CuSparseBackend : IDisposable
{
    ...
    // --- Function Pointers (Static Loading) ---
    ...
    private static void LoadLibraries()
    {
        try
        {
            ...
            {
                NativeExportHelper.TryGetExport(_libCudaRt, "cudaMalloc", out _cudaMalloc);
                NativeExportHelper.TryGetExport(_libCudaRt, "cudaFree", out _cudaFree);
                NativeExportHelper.TryGetExport(_libCudaRt, "cudaMemcpy", out _cudaMemcpy);
                NativeExportHelper.TryGetExport(_libCudaRt, "cudaDeviceSynchronize", out _cudaDeviceSynchronize);
            }
            ...
            {
                NativeExportHelper.TryGetExport(_libCuSparse, "cusparseCreate", out _cusparseCreate);
                NativeExportHelper.TryGetExport(_libCuSparse, "cusparseDestroy", out _cusparseDestroy);
                NativeExportHelper.TryGetExport(_libCuSparse, "cusparseCreateCsr", out _createCsr);
                NativeExportHelper.TryGetExport(_libCuSparse, "cusparseDestroySpMat", out _destroySpMat);
                NativeExportHelper.TryGetExport(_libCuSparse, "cusparseCreateDnVec", out _createDnVec);
                NativeExportHelper.TryGetExport(_libCuSparse, "cusparseDestroyDnVec", out _destroyDnVec);
                NativeExportHelper.TryGetExport(_libCuSparse, "cusparseSpMV_bufferSize", out _spmvBufferSize);
                NativeExportHelper.TryGetExport(_libCuSparse, "cusparseSpMV", out _spmv);
            }
...
// ============================================================================
// OPTIMIZED SPARSE KERNELS (SIMD-accelerated SpMV and SpMM)
// ============================================================================
